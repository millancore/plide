...
class TailwindColors
{
    ...
    public function bgTransform(string $tailwindBgTag) : ?string
    {
        if(!str_starts_with($tailwindBgTag, 'bg-')) {
            return '';
        }

        $colorParts = explode('-', $tailwindBgTag);

        return $this->palette[$colorParts[1]][$this->index[$colorParts[2]]];
    }
